...
/**
 * The hero's product shot: the top strip of the command centre.
 *
 * Deliberately free of scroll-reveal wrappers: on a phone one of these numbers
 * is the LCP element, and gating it behind an IntersectionObserver plus a
 * stagger delay meant LCP could not fire until hydration finished. The parent's
 * CSS entrance animation covers it instead, and that starts on the browser's
 * first painted frame.
 *
 * A deliberate slice rather than the whole dashboard — it is the most legible
 * part at hero size, and it sets up the full console in the section below
 * instead of duplicating it. Server component, no client JS.
 */
export function KpiStrip() {
  return (
    <div
      className="relative rounded-[22px] border border-line bg-white/80 p-2 shadow-[0_2px_4px_rgba(11,13,18,0.04),0_40px_80px_-40px_rgba(11,13,18,0.3)] backdrop-blur-xl"
    >
      <div className="flex items-center gap-2 px-3 py-2">
        ...
        <span className="hidden items-center gap-1.5 rounded-full border border-[#bfe9da] bg-[#e4f7f1] px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.1em] text-[#076b50] sm:inline-flex">
      ...
      <div className="grid grid-cols-2 gap-2 rounded-[16px] bg-surface-2 p-2 sm:grid-cols-3 lg:grid-cols-6">
        {KPIS.map((kpi) => {
          ...
          return (
            <div
              key={kpi.id}
              className="rounded-xl border border-line bg-white p-3 text-left"
            >
